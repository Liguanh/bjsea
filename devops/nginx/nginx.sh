...
{
    local nginx_log_path="$project_devops_runtime_dir/nginx-fpm"
    recursive_mkdir "$nginx_log_path"
    local nginx_docker_sites_fpm_conf_dir="$project_devops_persistent_dir/nginx-fpm-config"
    ...
    # nginx config
    args="$args -v $project_devops_nginx_dir/conf/nginx.conf:/etc/nginx/nginx.conf"

    # for the other sites
    args="$args -v $project_devops_nginx_dir/conf/extra/:/etc/nginx/extra"
