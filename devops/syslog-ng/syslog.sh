...
{
    local args=''
    local log_path="$project_devops_runtime_dir/syslog-ng"

    recursive_mkdir "$log_path"

    args="$args -v $log_path:/var/log"
    args="$args -p 514:514 -p 601:601/udp"
    args="$args -v $project_devops_syslogng_dir/conf/syslog-ng.conf:/etc/syslog-ng/conf.d/syslog-ng.conf"
    args="$args -v $project_devops_syslogng_dir/conf/sources.list:/etc/apt/sources.list"
