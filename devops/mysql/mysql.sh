...
function run_mysql()
{
    local mysql_data_dir="$project_devops_persistent_dir/mysql/data"
    local mysql_data_init_dir="$project_devops_mysql_path/mysql-init"
    local mysql_data_append_dir="$project_devops_mysql_path/mysql-append"
    local mysql_conf_dir="$project_devops_mysql_path/conf/"
...
{
    local cmd=$1
    run_cmd "docker exec -it $mysql_container bash -c 'cd $project_dir; $cmd'"
}
